...
class EcommerceApiTest extends TestCase
{
    ...
    public function test_whatsapp_settings_public()
    {
        $response = $this->getJson('/api/settings/whatsapp');
        $response->assertStatus(200)
                 ->assertJsonPath('success', true)
                 ->assertJsonPath('whatsapp_number', '51999999999');
    }

    public function test_whatsapp_settings_requires_authentication()
    {
        $response = $this->postJson('/api/settings/whatsapp', [
    ...
    public function test_whatsapp_settings_requires_admin_role()
    {
        $seller = User::find(2);
        $response = $this->actingAs($seller)->postJson('/api/settings/whatsapp', [
            'whatsapp_number' => '51988888888',
            'whatsapp_message_template' => 'Hola'
        ]);
        $response->assertStatus(403);
    ...
    public function test_whatsapp_settings_as_admin_succeeds()
    {
        $admin = User::find(1);

        // 1. Update settings successfully
        $response = $this->actingAs($admin)->postJson('/api/settings/whatsapp', [
            'whatsapp_number' => '51987654321',
            'whatsapp_message_template' => 'Hola {product_name}'
        ]);
        $response->assertStatus(200)
                 ->assertJsonPath('success', true);

        // Verify changes in database
        $this->assertEquals('51987654321', DB::select("SELECT `phone` FROM social_media_settings WHERE `type` = 'whatsapp'")[0]->phone);
        $this->assertEquals('Hola {product_name}', DB::select("SELECT `default_message` FROM social_media_settings WHERE `type` = 'whatsapp'")[0]->default_message);

        // 2. Try updating with non-numeric phone number (should fail validation)
        $response = $this->actingAs($admin)->postJson('/api/settings/whatsapp', [
            'whatsapp_number' => '5198765432a', // has letter
            'whatsapp_message_template' => 'Hola {product_name}'
        ]);
        $response->assertStatus(422)
